...
class Bagging():
    ...
    def baggem(self,df,m_size):
        ...
        for i in range(self.T):
            ...
            a = self.mytree.build_tree(S, A, Attributes, Label, 1, method)
            trees.append(a)
            
            # Calculate and print errors
        training_error = self.calc_error(trees, df)
        test_error = self.calc_error(trees, dftest)
...
for T in range(1,max_T):
    bagged = Bagging(T)
    training_error,test_error = bagged.baggem(df, m_size)
