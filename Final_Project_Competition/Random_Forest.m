...
mdl = TreeBagger(100, XTrain, TTrain, 'OOBPrediction', 'on', 'Method', 'classification');

% Evaluate on out-of-bag data (OOB)
...
% Predict on test set
YTestPred = predict(mdl, XValidation);
YTestPred = str2double(YTestPred);  % Convert predictions to numeric

% Assess performance
confusionMat = confusionmat(YTest, YTestPred);
accuracy = sum(diag(confusionMat)) / sum(confusionMat(:));
disp(['Test Accuracy: ', num2str(accuracy)]);

% Cross-validation using the entire dataset
mdlCV = fitctree(X, Y, 'CrossVal', 'on');
kfoldLoss(mdlCV)